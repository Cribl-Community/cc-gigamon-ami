...
- `src/queries/stackIds.ts` — **the one list of Stream object ids the Data Flow counters name**
  ...
  over modelled `cribl_metrics` rows to show nothing is double-counted. `src_events` is
  ...
  pair with another path — the planned JSON + Parquet dual-write — is not counted by its pipeline at
  all: `pipe.out_events` would count each event once per path, so its Processing figure is the
  destination's `total.out_events` (`DESTINATION_COUNTED_PATHS`); only the rest
  ...
  not counted. The queries count every stack listed — **including ones no release has shipped**:
  pack 0.1.0 is `unreleased` while `PACK_PUBLISHED` is false, and pack 0.2.0 is `planned`, its
  Raw HTTP ids taken from a design not yet built — so the live `METRICS_QUERY` already names
  objects no release has shipped. The ⓘ prose and stage
  headings name only stacks whose `status` is `running` or `offered` (`SHOWN_*`). Because
...
- **The dashboards are read-only against Search data: they submit Cribl Search jobs and read the
  ...
  panel reports as drifted** (`differs`): `read.ts` never compares a stored run's body with the
  manifest's, and a switch or Resume re-enables the body Cribl already holds, so until Re-apply
  the panel shows numbers from the old query under the new ⓘ. A change to

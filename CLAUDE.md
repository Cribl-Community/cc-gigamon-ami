...
- `src/queries/stackIds.ts` — **the one list of Stream object ids the Data Flow counters name**
  ...
  whether a routed path's rows carry it is unmeasured. The Destinations plate and the Lake card count
  only destinations that write `gigamon_ami` (`COUNTED_OUTPUTS`), so a copy to another dataset is
  not counted. The queries count every stack listed; the ⓘ prose and stage headings name only
  stacks whose `status` is `running` or `offered` (`SHOWN_*`). Because `gno_pipeline_c1h` runs
  `METRICS_QUERY` and `gno_lake_30d_c1d` can run `LAKE_TOTAL_QUERY`, a change to the list shows as
  drift on those schedules. *(Added 2026-09-24. Before it, the counters hard-coded the demo's
  objects.)*
...
| The ⓘ beside a number | `<PanelInfo>` / `<InfoTip>`. `InfoTip`'s pop is `position: fixed`, placed from the icon's rect at open time, flipped to the other side when it would leave the viewport and clamped horizontally, so a `.panel`'s `overflow: hidden` cannot cut it off. It opens on hover and focus and toggles on click/tap (the only reveal on a touch screen), and closes on scroll, resize, Escape and a press outside it. The icon carries the text as `role="note"` + `aria-label`. |

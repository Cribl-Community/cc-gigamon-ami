...
- `manifest.ts` — **the list is the object.** One record per scheduled search (`gno_lake_30d_c1d`, the Data Flow 30-day Lake total on `10 0 * * *`; `gno_sample_2m_c1h`, Field Explorer's 5,000-row sample on `7 * * * *`). Apply, teardown, status and the read path all iterate it, so a new entry is one record rather than five hand-kept lists. **The bodies are imported from `src/queries/*`, never retyped** — a panel's ⓘ stays truthful only while the thing that ran *is* the string the ⓘ shows. The dependency points one way: `src/queries/*` must not import this.

...
function render(run: () => Promise<unknown>) {
  act(() => {
    root.render(<GatedControl write="onboarding_stack.remove" label="Deploy" busyLabel="Deploying…" run={run} />)
  })
}
...
describe('GatedControl', () => {
  ...
  describe('a reason of the caller’s own', () => {
    const renderUnavailable = (unavailable: string | null, run = async () => {}) => {
      act(() => {
        root.render(
          <GatedControl write="onboarding_stack.remove" label="Deploy" busyLabel="Deploying…" unavailable={unavailable} run={run} />,
        )
      })
    }

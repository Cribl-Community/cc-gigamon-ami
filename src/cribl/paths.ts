...
export const API_CALLS: readonly ApiCall[] = [
  ...
  },
  {
    ...
    why: 'Read the pack’s route table: whether its three routes are installed — the two that send HTTP data to the JSON and Parquet datasets, and the sample’s — and whether the table is the one the installed version ships, which an upgrade does not replace when the table was changed after install. Read again after an upgrade, and before and after the route table is put back. Read only.',
  },
  {
    method: 'GET',
